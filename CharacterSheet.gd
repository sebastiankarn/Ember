...
onready var node_skill_points = get_node("HBoxContainer/VBoxContainer/Skills/SkillTree/1/SkillPoints")
...
func LoadSkills():
	for skill in get_tree().get_nodes_in_group("Skills"):
		if player.get("skill_" + skill.get_name()) == true:
			get_node("HBoxContainer/VBoxContainer/Skills/SkillTree/" 
			...
			if player.curLevel >= ImportData.skill_tree_data[skill.get_name()].ReqPlayerLevel:
				var texture_button = get_node("HBoxContainer/VBoxContainer/Skills/SkillTree/" 
...
func SpendSkillPoint(skill):
	if player.get("skill_" + skill) == true:
		...
		if player.skill_points < ImportData.skill_tree_data[skill].Cost:
			...
			var tween = get_node("HBoxContainer/VBoxContainer/Skills/SkillTree/" + 
			...
			if not unlocked_skills == []:
				for key in unlocked_skills:
					texture_button = get_node("HBoxContainer/VBoxContainer/Skills/SkillTree/" + 
