...
func set_quest_requirements(requirements):
	...
	for i in requirements.keys():
		...
		var requirement_node = template_quest_requirement.instantiate()
		...
		if type == "Kill":
			...
			if killed_count == amount:
				requirement_check_box.button_pressed = true
			else:
				requirement_check_box.button_pressed = false
...
func load_left_panel():
	...
	for i in PlayerData.quest_data.keys():
		...
		if (accepted and !abandoned and !completed):
			...
			quest_check_box.button_pressed = quest_requirements_completed
