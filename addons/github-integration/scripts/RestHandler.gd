...
func parse_body_data(body: PackedByteArray) -> Dictionary:
	...
	var parse_result = test_json_conv.parse(body.get_string_from_utf8())

	if parse_result.error == OK:
		return parse_result.result  # Returns the parsed data as a Dictionary
	else:
		printerr("JSON parsing error: ", parse_result.error_string)
		return {}  # Return an empty dictionary in case of error

# Parse the result body to a Dictionary
...
func request_user(token: String) -> void:
	...
	# Corrected request call with the appropriate number of arguments
	client.request(api_endpoints.user, temp_header, HTTPClient.METHOD_GET, "")
...
func request_user_repositories() -> void:
	...
	client.request(graphql_endpoint, UserData.header, HTTPClient.METHOD_POST, print_query(query))
...
func request_commit_gist(body: String) -> void:
	...
	client.request(api_endpoints.gist, UserData.header, HTTPClient.METHOD_POST, body)

func request_update_gist(gistid: String, body: String) -> void:
	requesting = REQUESTS.UPDATE_GIST
	client.request(api_endpoints.gist + "/" + gistid, UserData.header, HTTPClient.METHOD_PATCH, body)
...
func request_branch_contents(repository_name: String, repository_owner: String, branch: Dictionary) -> void:
	...
	var url = api_endpoints.repos + "/" + repository_owner + "/" + repository_name + "/git/trees/" + branch.target.tree.oid + "?recursive=1"
	client.request(url, UserData.header, HTTPClient.METHOD_GET, "")
...
func request_file_content(repository_owner: String, repository_name: String, file_path: String, branch_name: String) -> void:
	...
	var url = api_endpoints.repos + "/" + repository_owner + "/" + repository_name + "/contents/" + file_path + "?ref=" + branch_name
	client.request(url, UserData.header, HTTPClient.METHOD_GET, "")
...
func request_gitignore(repository_owner: String, repository_name: String, branch_name: String) -> void:
	...
	var url = api_endpoints.repos + "/" + repository_owner + "/" + repository_name + "/contents/.gitignore?ref=" + branch_name
	client.request(url, UserData.header, HTTPClient.METHOD_GET, "")
...
func request_pull_branch(ball_path: String, typeball_url: String, repo_disk_usage: float) -> void:
	...
	client.request(typeball_url, UserData.header, HTTPClient.METHOD_GET, "")
	set_process(true)
...
func request_collaborator(repository_owner: String, repository_name: String, collaborator_name: String, body: Dictionary) -> void:
	...
	var url = api_endpoints.repos + "/" + repository_owner + "/" + repository_name + "/collaborators/" + collaborator_name
	client.request(url, UserData.header, HTTPClient.METHOD_PUT, body_json)
...
func request_delete_resource(repository_owner: String, repository_name: String, path: String, body: Dictionary) -> void:
	...
	var json = JSON.new()
	var body_json = json.print(body)
	var url = api_endpoints.repos + "/" + repository_owner + "/" + repository_name + "/contents/" + path
	client.request(url, UserData.header, HTTPClient.METHOD_DELETE, body_json)

func request_delete_repository(repository_owner: String, repository_name: String) -> void:
	requesting = REQUESTS.DELETE_REPOSITORY
	var url = api_endpoints.repos + "/" + repository_owner + "/" + repository_name
	client.request(url, UserData.header, HTTPClient.METHOD_DELETE, "")
...
func request_create_new_branch(repository_owner: String, repository_name: String, body: Dictionary) -> void:
	...
	var json = JSON.new()
	var body_json = json.print(body)
	var url = api_endpoints.repos + "/" + repository_owner + "/" + repository_name + "/git/refs"
	client.request(url, UserData.header, HTTPClient.METHOD_POST, body_json)
...
func request_accept_invitation(invitation_id: int):
	...
	var url = api_endpoints.invitations + "/" + str(invitation_id)
	notifications_client.request(url, UserData.header, HTTPClient.METHOD_PATCH, "")

func request_decline_invitation(invitation_id: int):
	notifications_requesting = REQUESTS.DECLINE_INVITATION
	var url = api_endpoints.invitations + "/" + str(invitation_id)
	notifications_client.request(url, UserData.header, HTTPClient.METHOD_DELETE, "")
